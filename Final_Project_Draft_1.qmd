...
ggplot(df, aes(x = trip_distance, y = fare_amount)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Linear regression line without standard error
...
ggplot(df, aes(x = trip_time, y = total_amount)) +
  ...
  geom_smooth(method = "lm", color = "red", se = FALSE) +  # Add a linear regression line without standard error
...
We know that:
Standard Rate: The most common fare rate, applied to most metered trips within the city.
JFK: A flat fare for trips between JFK Airport and Manhattan. This rate is set to offer a predictable fare for travelers to and from this major airport.
Newark: A special fare rate for trips to Newark Liberty International Airport, which may include surcharges or a flat rate, depending on the jurisdictional agreements and regulations.
Nassau or Westchester: These rates apply to trips that go beyond the city limits to Nassau County or Westchester County. Fares might be metered at a different rate or negotiated based on the distance.
Negotiated Fare: Fares that are not determined by the meter but are agreed upon by the driver and passenger before the trip begins. This might apply to trips outside the typical service areas or under special circumstances.
Group Ride: Some jurisdictions may allow taxis to offer shared rides at a discounted rate. The specifics can vary, including how fares are split among passengers.
...
```{r, fig.align='left', fig.width=13, fig.height=9}
...
# Create the plot
ggplot() +
  geom_line(data = combined_data, aes(x = as.Date(day), y = NumberOfRides, colour = "Number of Rides"), size = 1) +
  geom_line(data = combined_data, aes(x = as.Date(day), y = TotalFare, colour = "Total Fare Amount"), size = 1) +
...
outlier_explanation <- "The decline in ridership observed in late January could be attributed to external factors such as sudden weatherchanges, particularly a freeze, which may not be explicitly captured in the dataset. Severe weather conditions can significantly impact transportation services, leading to a decrease in taxi usage as people opt for alternative modes of transportation or choose to stay indoors."
outlier_explanation
```
...
Below is a table that shows PULocationID and DOLocationID in relation to fare amount:
...
All possible distributions would be as followed:
```{r, echo = FALSE}
taxi_usage <- df %>%
...
Specifically for trips with distance 3.5 miles, 4.2 miles, and 2.8 miles, 
                                     20 minutes, 30 minutes, and 19 minutes,
                                     ratecodeIDs 1, 2, and 1 respectively.
