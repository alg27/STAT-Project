...
SELECT
  *,
  -- Assuming tpep_pickup_datetime and tpep_dropoff_datetime are in the format 'YYYY-MM-DD HH:MM:SS'
...
FROM
  jan_to_mar_taxi_2021
WHERE
  -- Filtering conditions
  strftime('%m', tpep_dropoff_datetime) < '04'
  AND strftime('%Y', tpep_dropoff_datetime) = '2021'
  AND (
    strftime('%H', tpep_dropoff_datetime) != '00'
    OR strftime('%M', tpep_dropoff_datetime) != '00'
    OR strftime('%S', tpep_dropoff_datetime) != '00'
  )
  AND (
    strftime('%H', tpep_pickup_datetime) != '00'
    OR strftime('%M', tpep_pickup_datetime) != '00'
    OR strftime('%S', tpep_pickup_datetime) != '00'
  )
  AND strftime('%Y', tpep_pickup_datetime) = '2021'
  AND trip_time > (10 / 60) -- Trip must be longer than 10 seconds
  AND trip_time < 600
