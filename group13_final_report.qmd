...
plot_grid <- function(data) {
  ...
  x_at <- seq(min(x_range), max(x_range), length.out = 5)
  y_at <- seq(min(y_range), max(y_range), length.out = 5)

  for (x in x_at) {
    grid.text(round(x, 1), x = unit(x, "native"), y = unit(min(y_range) - 0.05*(max(y_range) - min(y_range)), "native"), just = "top")
  }

  for (y in y_at) {
    grid.text(round(y, 1), x = unit(min(x_range) - 0.05*(max(x_range) - min(x_range)), "native"), y = unit(y, "native"), just = "right")
  }
