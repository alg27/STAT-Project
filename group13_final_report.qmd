...
plot_grid <- function(data) {
  grid.newpage()
  # Adjust viewport to include margins
  vp <- viewport(layout = grid.layout(1, 1), width = 0.9, height = 0.8, x = 0.5, y = 0.5)
  pushViewport(vp)
  ...
  for (i in 1:length(x_ticks)) {
    grid.text(x_labels[i], x = unit(x_ticks[i], "npc"), y = unit(-0.02, "npc"), just = "top")
  }

  for (i in 1:length(y_ticks)) {
    grid.text(y_labels[i], x = unit(-0.02, "npc"), y = unit(y_ticks[i], "npc"), just = "right")
  }
