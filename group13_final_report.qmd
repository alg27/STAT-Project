...
plot_grid <- function(data) {
  ...
  x_range <- range(data$trip_distance)
  y_range <- range(data$fare_amount)

  # Basic scatter plot
  for (i in unique(data$RatecodeID)) {
    subset_data <- subset(data, RatecodeID == i)
    points <- pointsGrob(subset_data$trip_distance, subset_data$fare_amount,
                         pch = 16, gp = gpar(col = rainbow(length(unique(data$RatecodeID)))[i], cex = 0.8))
    grid.draw(points)
  }

  # Adding axes
  grid.xaxis(at = unit(seq(from = min(x_range), to = max(x_range), length.out = 5), "native"), 
             label = seq(from = min(x_range), to = max(x_range), length.out = 5))
  grid.yaxis(at = unit(seq(from = min(y_range), to = max(y_range), length.out = 5), "native"),
             label = seq(from = min(y_range), to = max(y_range), length.out = 5))

  # Labels and titles
  grid.text("Distance (miles)", x = unit(0.5, "npc"), y = unit(-0.1, "npc"), just = "centre")
  grid.text("Fare ($)", x = unit(-0.1, "npc"), y = unit(0.5, "npc"), just = "centre", rot = 90)
  grid.text("Dynamic Fare Analysis by Rate Code ID", x = unit(0.5, "npc"), y = unit(1.05, "npc"), just = "centre")
