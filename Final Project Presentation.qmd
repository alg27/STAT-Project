...
# Define UI
ui <- fluidPage(
  titlePanel("Circular Plot with Hour Markers"),
  mainPanel(
    plotOutput("circularPlot", width = "400px", height = "400px")
  )
)
...
server <- function(input, output, session) {
  output$circularPlot <- renderPlot({
    ...
    ggplot(sector_df) +
      ...
      scale_fill_identity() +
      geom_text(data = hour_labels, aes(x = 1.2 * cos(angle), y = 1.2 * sin(angle), label = hour), size = 4) +  # Adjust label positions
      theme_void() +
      coord_fixed() +
      theme(legend.position = "none") +
      labs(title = "Circular Plot with 24 Sectors and Hour Labels")
  })
}
